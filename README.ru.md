...
Используете NestJS? Есть готовая обёртка: [platega-sdk-nestjs](https://github.com/tsforge/platega-sdk-nestjs).
...
**Нашли ошибку?** Создайте [Issue](https://github.com/tsforge/platega-sdk/issues/new) — опишите, что делали, что ожидали и что получили (код ошибки, версия SDK и Node). Пожалуйста, не выкладывайте в issue свои `X-MerchantId`/`X-Secret` и реальные данные транзакций.

**Хотите предложить изменение?** Напрямую в репозиторий пушить нельзя — изменения принимаются через Merge Request из форка:

1. **Сделайте форк** репозитория — кнопка «Fork» на странице [tsforge/platega-sdk](https://github.com/tsforge/platega-sdk).
2. **Склонируйте свой форк** и создайте ветку:

    ```bash
    git clone git@github.com:<ваш-логин>/platega-sdk.git
    cd platega-sdk
    npm install
...
3. **Внесите изменения** и убедитесь, что всё зелёное:
    ...
    Новую логику покрывайте тестами (`*.spec.ts` рядом с файлом), комментарии в коде — на английском.

4. **Запушьте ветку в свой форк** и откройте Merge Request в `main` основного репозитория. В описании укажите, что и зачем меняли; если MR закрывает issue — сошлитесь на него (`Closes #N`).
